...
def checker(x,omin=1000,omax=2000):
    if x >= omax:
        x = omax
    if x <= omin:
        x = omin
...
#########################################################################
    ...
    if tspeed == 0:
        ...
        print(angleMeter.get_kalman_roll(),angleMeter.get_kalman_pitch())
        errx = -angleMeter.get_kalman_pitch() - xslope
        erry = angleMeter.get_kalman_roll() - yslope
        ...
        ms1 = checker(int(speeder(tspeed + speed1,out_min=min1,out_max=max1) - pidx + pidy))
        ms2 = checker(int(speeder(tspeed + speed2,out_min=min2,out_max=max2) - pidx - pidy))
        ms3 = checker(int(speeder(tspeed + speed3,out_min=min3,out_max=max3) + pidx - pidy))
        ms4 = checker(int(speeder(tspeed + speed4,out_min=min4,out_max=max4) + pidx + pidy))
