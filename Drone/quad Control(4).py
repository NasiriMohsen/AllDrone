...
#########################################################################
    ...
    if tspeed == 0:
        ...
        pidxx = pid(currtime,previoustimex,lastix,errx,lasterrx,Kpy,Kiy,Kdy)
        currtime = int(round((time.time()-stime)*1000))
        pidyy = pid(currtime,previoustimey,lastiy,erry,lasterry,Kpy,Kiy,Kdy)
        pidx = int(pidxx[0]*10000)/10000
        pidy = int(pidyy[0]*10000)/10000
        print("pidy: ",pidy," pidx: ",pidx)
        lastix = pidxx[1]
        lastiy = pidyy[1]
        previoustimex = pidxx[2]
        previoustimey = pidyy[2]
        lasterrx = errx
        lasterry = erry 
        motors.position(
